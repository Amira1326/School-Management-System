<?php
session_start(); // يجب أن تكون أول شيء في ملف PHP
...
// الاتصال بقاعدة البيانات لجلب الإحصائيات الحقيقية للمدير
$conn = new mysqli('localhost', 'root', '', 'aljood_center');
if ($conn->connect_error) {
    // في بيئة الإنتاج، يفضل تسجيل الخطأ بدلاً من عرضه مباشرة للمستخدم
    // error_log("فشل الاتصال بقاعدة البيانات: " . $conn->connect_error);
    die("فشل الاتصال: " . $conn->connect_error);
}

// جلب الإحصائيات الخاصة بالمدير
...
$paymentsReceived = $conn->query("SELECT SUM(amount) AS total FROM payments")->fetch_assoc()['total'] ?? 0;

$loginTime = date("h:i A"); // الحصول على الوقت الحالي
$avatarFile = 'avtar.jpg'; // مسار صورة الأفاتار للمدير (تأكد من وجود هذا الملف)

// تحديد الصفحة الحالية لتفعيل عنصر القائمة الجانبية
...
<body>
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="menu">
            <a href="dashboard.php" class="menu-item <?= ($currentPage == 'dashboard.php') ? 'active' : '' ?>" data-tooltip="الرئيسية">
                <span class="material-icons">dashboard</span>
                <span class="label">الرئيسية</span>
            </a>
            <a href="classes.php" class="menu-item <?= ($currentPage == 'classes.php') ? 'active' : '' ?>" data-tooltip="الفصول">
                <span class="material-icons">school</span>
                <span class="label">الفصول</span>
            </a>
            <a href="teachers.php" class="menu-item <?= ($currentPage == 'teachers.php') ? 'active' : '' ?>" data-tooltip="المعلمين">
                <span class="material-icons">group</span>
                <span class="label">المعلمين</span>
            </a>
            <a href="students.php" class="menu-item <?= ($currentPage == 'students.php') ? 'active' : '' ?>" data-tooltip="الطلاب">
                <span class="material-icons">person</span>
                <span class="label">الطلاب</span>
            </a>
            <a href="payments.php" class="menu-item <?= ($currentPage == 'payments.php') ? 'active' : '' ?>" data-tooltip="الرسوم">
                <span class="material-icons">payments</span>
                <span class="label">الرسوم</span>
            </a>
            <a href="grades_dashboard.php" class="menu-item" data-tooltip="الدرجات">
                <span class="material-icons">grading</span>
                <span class="label">الدرجات</span>
            </a>
            <a href="absence_select.php" class="menu-item <?= ($currentPage == 'absence_select.php') ? 'active' : '' ?>" data-tooltip="الحضور">
        ...
        <button class="dark-btn material-icons" aria-label="تبديل الوضع الليلي">brightness_2</button>
            ...
            <section class="banner">
                <div class="banner-content">
                    <h2><?php echo get_greeting_based_on_time(); ?> يا أستاذ <?php echo $adminName; ?> 👋</h2>
                    <p>تم تسجيل دخولك الساعة: <?php echo $loginTime; ?></p>
                </div>
                <img class="avatar" src="<?php echo $avatarFile; ?>" alt="صورة الملف الشخصي للمدير">
            </section>
    ...
    <div class="carousel-item">
        <img src="sud.png" alt="صورة طلاب في الفصل"> </div>
                ...
                <section class="cards">
                    ...
                    <div class="card" data-icon="group">
                        <div class="card-info">
                            <h3><?= $teachersCount ?></h3>
                            <p>إجمالي المعلمين</p>
                        </div>
